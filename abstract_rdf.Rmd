...
```{r}
cluster_dist <- my_data %>% select(age, legal_tradition, assets, kkm, pcrdbgdp, stmktcap, gdp_growth_annual) %>% dist(method = "euclidean")

my_clusters <- hclust(cluster_dist, method = "ward.D")
